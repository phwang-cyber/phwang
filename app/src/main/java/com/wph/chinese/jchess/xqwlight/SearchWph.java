...
package com.wph.chinese.jchess.xqwlight;

public class SearchWph {
    ...
    private static final int MAX_GEN_MOVES = PositionWph.MAX_GEN_MOVES;
    private static final int MATE_VALUE = PositionWph.MATE_VALUE;
    private static final int BAN_VALUE = PositionWph.BAN_VALUE;
    private static final int WIN_VALUE = PositionWph.WIN_VALUE;

    private int hashMask, mvResult, allNodes, allMillis;
    private HashItemWph[] hashTable;
    PositionWph pos;
    int[] historyTable = new int[4096];
    int[][] mvKiller = new int[LIMIT_DEPTH][2];

    public SearchWph(PositionWph pos, int hashLevel) {
        this.pos = pos;
        hashMask = (1 << hashLevel) - 1;
        hashTable = new HashItemWph[hashMask + 1];
        for (int i = 0; i <= hashMask; i++) {
            hashTable[i] = new HashItemWph();
        }
    }

    private HashItemWph getHashItem() {
        return hashTable[pos.zobristKey & hashMask];
    }

    private int probeHash(int vlAlpha, int vlBeta, int depth, int[] mv) {
        HashItemWph hash = getHashItem();
    ...
    private class SortItem {
        ...
        SortItem(int mvHash) {
            ...
            UtilWph.shellSort(mvs, vls, 0, moves);
            index = 0;
            singleReply = moves == 1;
        }
    ...
    private int searchQuiesc(int vlAlpha_, int vlBeta) {
        ...
        if (pos.inCheck()) {
            ...
            UtilWph.shellSort(mvs, vls, 0, genMoves);
        } else {
            vl = pos.evaluate();
            if (vl > vlBest) {
                if (vl >= vlBeta) {
            ...
            UtilWph.shellSort(mvs, vls, 0, genMoves);
            for (int i = 0; i < genMoves; i++) {
                if (vls[i] < 10 || (vls[i] < 20 && PositionWph.HOME_HALF(PositionWph.DST(mvs[i]), pos.sdPlayer))) {
                    genMoves = i;
                    break;
                }
            }
        ...
        while ((mv = sort.next()) > 0) {
            ...
            if (vl > vlBest) {
                ...
                if (vlBest > -WIN_VALUE && vlBest < WIN_VALUE) {
                    vlBest += (PositionWph.random.nextInt() & RANDOM_MASK) -
                            (PositionWph.random.nextInt() & RANDOM_MASK);
